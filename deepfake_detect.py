...
from dataprepare import DeepFakeFrame
...
def train(epoch, model, optimizer, criterion, trainloader, scheduler, device, args):
    ...
    for i, (images, labels, origin) in enumerate(tq):
...
def validating(epoch, model, optimizer, criterion, valloader, scheduler, device, args, best_epoch, best_kaggle):
    total_loss = 0
    loss = 0
    pred = []
    real = []
    ...
    with torch.no_grad():
        for i, (images, labels, origin) in enumerate(tqdm(valloader, ascii=True, ncols=50)):
        ...
        if kaggle <= best_kaggle:
            best_epoch = epoch
            best_kaggle = kaggle
            torch.save(model.state_dict(), "best_model.pkl")

    return best_epoch, best_kaggle
...
def main(args):
    ...
    trainset = DeepFakeFrame("/data/songzhu/deepfake", split='training', train_val_ratio=0.9, transform=train_transform)
    ...
    for epoch in range(args.n_epochs):
        ...
        best_epoch, best_kaggle = validating(epoch, net, optimizer, criterion_2, valloader, scheduler, device, args,
                                             best_epoch=best_epoch, best_kaggle=best_kaggle)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', default=5, help='delimited list input of GPUs', type=int)
